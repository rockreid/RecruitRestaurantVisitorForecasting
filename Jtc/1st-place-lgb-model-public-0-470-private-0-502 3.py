...
date_info['holiday_flg2'] = pd.to_datetime(date_info['visit_date']).dt.dayofweek #dt.dayofweek 用法值得借鉴
date_info['holiday_flg2'] = ((date_info['holiday_flg2']>4) | (date_info['holiday_flg']==1)).astype(int) # 两种holiday flag

air_store['air_area_name0'] = air_store['air_area_name'].apply(lambda x: x.split(' ')[0])
lbl = LabelEncoder()
air_store['air_genre_name'] = lbl.fit_transform(air_store['air_genre_name'])    # 把店铺类型，区域等文字信息通过LabelEncoder变成无大小关系的数字
air_store['air_area_name0'] = lbl.fit_transform(air_store['air_area_name0'])

data['visitors'] = np.log1p(data['visitors'])  # 数据平滑处理:针对偏度比较大的数据用log1p函数进行转化使其更加服从高斯分布
...
def get_store_visitor_feat(label, key, n_day):
    ...
    return result
    # 之后会取 前1000,56,28,14天的visitors统计值


def get_store_exp_visitor_feat(label, key, n_day):    # exp 平方的意思？
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0],x)) #  距离enddate的时间差值
    data_temp['weight'] = data_temp['visit_date'].apply(lambda x: 0.985**x)  #weight 取法？
...
def get_store_week_feat(label, key, n_day):  # 该DOM的所有统计值
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    return result
   #   n_day之后会取 前1000,56,28,14天的值

def get_store_week_diff_feat(label, key, n_day):    #求与前一天的差值
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result = data_temp.set_index(['store_id','visit_date'])['visitors'].unstack()    #unstack()は行から列へのピボット
...
def get_store_all_week_feat(label, key, n_day): #每一条reord 都该店铺1DOW～7DOW的统计值
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    for i in range(7):
        ...
        result_sub = result_sub.add_prefix(str(i))  # 列的名字前加1～7
...
def get_store_week_exp_feat(label, key, n_day):
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0],x))   # visitors差值
    data_temp['visitors2'] = data_temp['visitors']                            #visotrs值
    result = None
    for i in [0.9,0.95,0.97,0.98,0.985,0.99,0.999,0.9999]:    #尝试各种权重参数
...
def get_store_holiday_feat(label, key, n_day):   # 按照是否是holiday，holiday2做了各项统计
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
